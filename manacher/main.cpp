...
std::string removeHashes(std::vector<int> P, std::vector<string> T) {
...
std::vector<int> manacherP(std::string T) {
    ...
    // P = 00000000000000000
    // P = 00103050705030100
    for (int i = 1; i < len; i++) { // Move over all the string 

        int mirr = 2*c - i;

        if(i < r) { // This shit just speeds up the process, so its good for time complexity
            P[i] = std::min(r - i, P[mirr]);
        } 

        while(T[i + (1 + P[i])] == T[i - (1 + P[i])]) { // Move to the previous and next character to see if there are equals, if so, P[i] increases its value
            //std::cout << T[i] <<" Prev: " <<T[i + (1 + P[i])] << " Next: " << T[i - (1 + P[i])] << std::endl;
            P[i]++;
        }

        if(i + P[i] > r) { // If the new Right is greater than the accumulated R
